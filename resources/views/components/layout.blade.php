...
<nav class="sticky top-0 z-[100] border-b border-orange-500/10 bg-orange-600 nav-blur transition-all duration-500 shadow-2xl shadow-orange-900/10">
    <div class="max-w-[1440px] mx-auto px-6 lg:px-12 py-4 flex items-center justify-between">
        ...
        <div class="flex items-center gap-10">
            <a href="{{ route('home') }}" class="group flex items-center gap-3 no-underline">
                ...
                <div class="flex flex-col leading-none">
                    <h2 class="text-white font-black text-2xl tracking-tighter uppercase mb-0">Crave<span class="text-orange-200 italic">Cart</span></h2>
                    <span class="text-[9px] font-bold text-orange-100 uppercase tracking-[0.3em] opacity-80">Studio Hub</span>
                </div>
            </a>
        ...
        <div class="flex items-center gap-6">
            ...
            <div class="relative hidden xl:block group">
                <i class="fa-solid fa-magnifying-glass absolute left-4 top-1/2 -translate-y-1/2 text-orange-200/60 group-focus-within:text-white transition-colors"></i>
                <input type="text" placeholder="Find essentials..." class="pl-11 pr-4 py-2.5 bg-white/10 border border-white/10 rounded-2xl text-sm font-semibold text-white placeholder-orange-100/50 focus:outline-none focus:bg-white/20 w-44 focus:w-64 transition-all duration-500">
            </div>
            
            <div class="flex items-center gap-4">
                @auth
                    <!-- CART -->
                    <a href="{{ route('cart.index') }}" class="text-white hover:text-orange-200 transition-all relative p-2">
                        <i class="fa-solid fa-cart-shopping text-xl"></i>
                        <span class="absolute top-0 right-0 w-4 h-4 bg-white text-orange-600 text-[9px] font-black rounded-full flex items-center justify-center shadow-sm">0</span>
                    </a>
                    ...
                    <div class="flex items-center gap-3 bg-white/10 p-1.5 pr-4 rounded-2xl border border-white/10 group transition-all">
                        ...
                        <div class="flex flex-col">
                            ...
                            <form action="{{ route('logout') }}" method="POST" class="m-0 p-0">
                                @csrf
                                <button type="submit" class="text-orange-200 hover:text-white text-[8px] font-black uppercase tracking-tighter transition-colors block text-left">
                                    [Logout]
                                </button>
                            </form>
                        </div>
                ...
                @else
                    ...
                    <a href="{{ route('chooseRole') }}" class="bg-white text-orange-600 px-8 py-3.5 rounded-2xl font-black uppercase text-[10px] tracking-widest shadow-xl hover:bg-slate-900 hover:text-white hover:-translate-y-1 transition-all active:scale-95 no-underline">
                        GET STARTED
                    </a>
                @endauth
            </div>
    ...
    <div class="max-w-[1440px] mx-auto px-6 lg:px-12 grid grid-cols-1 lg:grid-cols-12 gap-16 lg:gap-8 relative z-10">
        <div class="lg:col-span-5 space-y-8">
            ...
            <div class="flex gap-4">
                <a href="#" class="w-12 h-12 rounded-2xl bg-white/5 border border-white/5 flex items-center justify-center hover:bg-orange-600 hover:border-orange-600 transition-all group text-white no-underline">
        ...
        <div class="lg:col-span-3">
            <h4 class="uppercase tracking-[0.4em] text-[10px] font-black mb-10 text-orange-500">System Nav</h4>
        ...
        <div class="lg:col-span-3">
            <h4 class="uppercase tracking-[0.4em] text-[10px] font-black mb-10 text-orange-500">The Studio</h4>
