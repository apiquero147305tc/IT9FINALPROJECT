    ...
    <section class="min-h-screen flex items-center justify-center bg-[#FDFCFB] px-6 py-20 relative overflow-hidden">
        ...
        <div class="relative z-10 w-full max-w-[550px]">
            <div class="text-center mb-10">
                <div class="inline-flex bg-red-600 p-3 rounded-2xl shadow-xl shadow-red-200 mb-6 -rotate-3 group hover:rotate-0 transition-transform duration-500">
                    <span class="text-2xl">🤝</span>
                </div>
                <h2 class="text-4xl md:text-5xl font-black uppercase tracking-tighter text-slate-900 leading-none">
                    Create <span class="text-red-600">Account.</span>
                </h2>
            ...
            <div class="bg-white p-8 md:p-12 rounded-[50px] border border-slate-100 shadow-2xl shadow-slate-200/50">
                ...
                <form action="{{ $role === 'buyer' ? route('buyer.register.post') : route('seller.register.post') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="space-y-6">
                        <div class="space-y-2">
                            ...
                            <input type="text" name="name" value="{{ old('name') }}" required
                                class="w-full bg-slate-50 border-none rounded-2xl p-4 text-sm font-semibold focus:ring-2 focus:ring-red-600 outline-none transition-all placeholder-slate-300"
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="relative">
                                ...
                                <button type="button" onclick="togglePassword()" class="absolute right-4 top-1/2 -translate-y-1/2 text-slate-300 hover:text-red-600 transition-colors">
                    ...
                    <div class="pt-6 border-t border-slate-50">
                        <label class="text-[10px] font-black uppercase tracking-[0.2em] text-red-600 mb-4 block">Account Protocol</label>
                        
                        <div class="relative group">
                            <select name="role" id="roleSelect" onchange="toggleRoleFields()"
                                class="w-full bg-slate-900 text-white border-2 border-slate-900 rounded-2xl p-4 pr-12 text-sm font-bold focus:ring-4 focus:ring-red-600/20 focus:border-red-600 outline-none transition-all appearance-none cursor-pointer hover:bg-slate-800">
                    ...
                    <div id="seller-info" class="space-y-6 pt-4 hidden">
                        <div class="bg-red-50/50 p-6 rounded-[35px] space-y-6 border border-red-100">
                            <div class="space-y-2">
                                <label class="text-[10px] font-black uppercase tracking-widest text-red-600 ml-2">Shop Name</label>
                                <input type="text" name="shop_name" value="{{ old('shop_name') }}"
                                    class="w-full bg-white border-none rounded-xl p-3 text-sm font-semibold outline-none focus:ring-2 focus:ring-red-600">
                            </div>
                            <div class="grid grid-cols-2 gap-4">
                                <div class="space-y-2">
                                    <label class="text-[10px] font-black uppercase tracking-widest text-red-600 ml-2">Contact</label>
                                    <input type="text" name="contact_number" value="{{ old('contact_number') }}"
                                        class="w-full bg-white border-none rounded-xl p-3 text-sm font-semibold outline-none focus:ring-2 focus:ring-red-600">
                                </div>
                                <div class="space-y-2">
                                    <label class="text-[10px] font-black uppercase tracking-widest text-red-600 ml-2">Age</label>
                                    <input type="number" name="age" min="18" value="{{ old('age') }}"
                                        class="w-full bg-white border-none rounded-xl p-3 text-sm font-semibold outline-none focus:ring-2 focus:ring-red-600">
                                </div>
                            </div>
                            <div class="space-y-2">
                                <label class="text-[10px] font-black uppercase tracking-widest text-red-600 ml-2">Identification Image</label>
                                <input type="file" name="valid_id" 
                                    class="w-full text-[10px] font-black text-slate-400 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-[10px] file:font-black file:uppercase file:bg-red-600 file:text-white hover:file:bg-slate-900 cursor-pointer">
                            </div>
                        </div>
                    </div>
                    ...
                    <div id="student-info" class="space-y-6 pt-4">
                        <div class="bg-slate-50 p-6 rounded-[35px] space-y-6 border border-slate-100">
                            ...
                            <div id="custom-budget-input" class="space-y-2 hidden">
                                ...
                                <input type="number" name="custom_budget" placeholder=""
                                    class="w-full bg-white border-none rounded-xl p-3 text-sm font-semibold outline-none focus:ring-2 focus:ring-red-600 transition-all">
                            </div>
                        </div>
                    </div>

                    <button type="submit" class="w-full bg-slate-900 text-white py-5 rounded-2xl font-black uppercase tracking-[0.2em] text-[11px] hover:bg-red-600 transition-all shadow-xl shadow-slate-200 active:scale-95 border-none cursor-pointer mt-8">
                      SIGN UP
                    </button>
                </form>
                ...
                <div class="mt-10 text-center">
                    <p class="text-slate-400 font-bold uppercase text-[10px] tracking-widest">
                        Returning User? 
                        <a href="{{ route('login') }}" class="text-red-600 no-underline border-b-2 border-red-600/20 hover:border-red-600 transition-all ml-1">Log In</a>
                    </p>
                </div>
            </div>
        </div>
